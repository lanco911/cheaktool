...
def open_dir(input_dir_name, open_dir_path):
    """打开指定路径'open_dir_path'下的指定文件夹input_text"""
    path_list = os.listdir(open_dir_path)
    # 大到小排序path_list
    path_list.sort(reverse=True)
    dir_path = []  # 存放送审文件夹路径
    for i in path_list:
        if input_dir_name in i and os.path.isdir(open_dir_path + '/' + i):  # 判断input_text是否存在及是否为文件夹
            dir_path = open_dir_path + '/' + i
            break
    # 判断dir_path是否为空
    if dir_path:
